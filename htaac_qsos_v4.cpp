...
const std::string diagram_name = "s3v110c700-1-testing-12";
...
const float anneal_timescale = 189.8f;
const float anneal_offset = 0.34f;
...
int main() {
  ...
  for(int rep = 0; rep < number_of_repetitions; rep++){
    ...
    for(int epoch = 0; epoch < number_of_epochs; epoch++){
      float rep_factor = (float) (rep * 2) / number_of_repetitions;
      float anneal_factor = (float) epoch / anneal_timescale + anneal_offset;
      //Initialize state
      ...
      float loss = proper_loss + coeff * anneal_factor * contraint_loss;
  ...
  graph_results(diagram_name, number_of_repetitions, number_of_epochs, rounded_scores, unrounded_scores, loss_scores, constraint_scores, elapsed_time.count());
